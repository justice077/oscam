				...
				switch (er->rc) {
					case 0: 
						if (er->selected_reader) {
							...
							fprintf(ecmtxt, "%s: %s\n", cfg.dvbapi_ecm_infomode == ECMINFO_MODE_OSCAM ? "from":"address",is_cascading_reader(er->selected_reader) ? er->selected_reader->device : "local");
						fprintf(ecmtxt, "%s: %s\n", cfg.dvbapi_ecm_infomode == ECMINFO_MODE_OSCAM ? "protocol":"using",reader_get_type_desc(er->selected_reader, 1));
					...
					case 1:	
						...
						fprintf(ecmtxt, "%s: cache1\n",cfg.dvbapi_ecm_infomode == ECMINFO_MODE_OSCAM ? "from" : "address");
						fprintf(ecmtxt, "%s: none\n",cfg.dvbapi_ecm_infomode == ECMINFO_MODE_OSCAM ? "protocol" : "using");
						break;

					case 2:	
						fprintf(ecmtxt, "reader: Cache\n");
						fprintf(ecmtxt, "%s: cache2\n",cfg.dvbapi_ecm_infomode == ECMINFO_MODE_OSCAM ? "from" : "address");
						fprintf(ecmtxt, "%s: none\n",cfg.dvbapi_ecm_infomode == ECMINFO_MODE_OSCAM ? "protocol" : "using");
						break;

					case 3:	
						fprintf(ecmtxt, "reader: Cache\n");
						fprintf(ecmtxt, "%s: cache3\n",cfg.dvbapi_ecm_infomode == ECMINFO_MODE_OSCAM ? "from" : "address");
						fprintf(ecmtxt, "%s: none\n",cfg.dvbapi_ecm_infomode == ECMINFO_MODE_OSCAM ? "protocol" : "using");
						break;
				}
